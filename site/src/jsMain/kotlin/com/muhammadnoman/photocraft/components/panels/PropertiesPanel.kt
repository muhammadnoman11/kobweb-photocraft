...
fun PropertiesPanel(
    ...
    val requiresImage = activeTool in listOf(
        ActiveTool.CROP, ActiveTool.FILTERS, ActiveTool.ADJUSTMENTS
    )
    val isDisabled = requiresImage && !hasImage

    Column(
        modifier = PropertiesPanelStyle.toModifier()
    ) {
        if (isVisible) {
            ...
            // Scrollable Content
            Box(
                modifier = PanelContentStyle.toModifier()
                    .flexGrow(1)
                ...
                if (isDisabled) {
                    Column(
                        modifier = Modifier
                            ...
                            .zIndex(20)
                            .fillMaxWidth()
                            .backgroundColor(Color.rgba(15, 15, 15, 220))
                            .padding(32.px, 24.px)
                ...
                Box(
                    modifier = Modifier
                        .then(if (isDisabled) Modifier.opacity(0.4f).pointerEvents(PointerEvents.None) else Modifier)
                    ...
                    when (activeTool) {
                        ...
                        ActiveTool.TEXT -> TextPanel(canvas = canvas, onChanged = onChanged)
                        ActiveTool.STICKERS -> StickersPanel(canvas = canvas, onChanged = onChanged)
                        ActiveTool.SHAPES -> ShapesPanel(canvas = canvas, onChanged = onChanged)
                        ActiveTool.LAYERS -> LayersPanel(canvas = canvas, onChanged = onChanged)
                        ActiveTool.NONE -> {}
                    }
                }
            }
